...
<script>
   $(function(){
	   ...
	   $("#selPage").change(function() {
			...
			location.href = "/project/board/shadow/list.do?page=" + $(this).val();
		...
		<section id = "detail">
			...
			<table id="tblList" class="table table-bordered table-striped long">
				...
				<tr <c:if test="${dto.notice == 1}">style='background-color:#C8EBFA;'</c:if>>
					...
					<td>					
					<!-- 제목 -->
					<a href="/project/board/shadow/view.do?shadowseq=${dto.shadowseq}&isSearch=${isSearch}&column=${column}&word=${word}">${dto.subject}</a>
			...
			<!-- 검색 창 -->
			<div id="search">
				<form method="GET" action="/project/board/shadow/list.do">
			...
			<div id="dao" class="btns long">
				...
				<input type="button" value="글쓰기" class="btn btn-danger"
					onclick="location.href='/project/board/shadow/add.do';">
				<%-- </c:if> --%>
			</div>
